...
2. Check for an existing Section/Frame for this component — create one if absent
3. Build all variants as individual components, 1:1 with the RN Storybook spec, using hex fallback colors (Blueprint Base variables cannot bind cross-file)
4. Combine with `figma.combineAsVariants(allComponents, page)` — **then immediately run a grid layout pass** (see below), because `combineAsVariants` stacks all children at (0,0)
5. After each `figma_execute` call, take a screenshot and check:
   - All variant axes represented and readable
   - Alignment, spacing, and visual balance match the RN Storybook
   - No overlapping nodes
6. Iterate up to 3 times to fix issues, then take a final verification screenshot
